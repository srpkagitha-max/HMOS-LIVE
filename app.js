...
const ADMIN_SCREENS = new Set([
  ...
  "pending-admissions","admissions-home","kitchen","entry-exit","complaints-admin","pdf-reports","approvals",
  "admin-notifications","audit-logs","recycle-bin","backup-restore","system-health","settings",
  "dashboard-settings","admission-fee-settings","admin-login-settings","manual-admission"
]);
const STUDENT_SCREENS = new Set([
  "student-dashboard","student-profile","student-fees","student-attendance","student-entry-exit",
...
const SCREEN_PARENTS = {
  ...
  "admin-notifications":"admin-home","audit-logs":"admin-home","recycle-bin":"admin-home","backup-restore":"admin-home",
  "system-health":"admin-home","settings":"admin-home","dashboard-settings":"settings","admission-fee-settings":"settings",
...
function saveUiScreen(){
  try{
    if(state.instituteSession || state.studentSession || state.adminAuthenticated){
      sessionStorage.setItem(UI_SCREEN_KEY,state.screen);
    }
  }catch{}
}
function restoreUiScreen(){ try{return sessionStorage.getItem(UI_SCREEN_KEY)||"";}catch{return "";} }
function clearUiScreen(){ try{sessionStorage.removeItem(UI_SCREEN_KEY);}catch{} }
...
function saveInstituteSession(session, remember=false){
  const safe = { instituteId:session.instituteId, instituteCode:session.instituteCode, instituteName:session.instituteName, hostelType:session.hostelType||"hostel", ownerPhone:session.ownerPhone||"", ownerEmail:session.ownerEmail||"", city:session.city||"", address:session.address||"", payeeName:session.payeeName||"", upiId:session.upiId||"", paymentContact:session.paymentContact||"", paymentQrDataUrl:session.paymentQrDataUrl||"", shareInstituteId:session.shareInstituteId||"", shareInstitutePassword:session.shareInstitutePassword||"", status:session.status||"active", subscriptionStatus:session.subscriptionStatus||"active", subscriptionEnd:session.subscriptionEnd||null, mustChangePassword:Boolean(session.mustChangePassword), upiId:session.upiId||"", branding:session.branding||null, shareInstituteId:session.shareInstituteId||"", shareInstitutePassword:session.shareInstitutePassword||"" };
...
function field(id,label,type="text",placeholder="",value="",extra=""){const input=`<input id="${id}" type="${type}" placeholder="${placeholder}" value="${esc(value)}" ${extra}/>`;return type==="password"?`<label class="field password-field" for="${id}"><span>${label}</span><div class="password-input-wrap">${input}<button class="password-toggle" type="button" data-toggle-password="${id}" aria-label="Show password">Show</button></div></label>`:`<label class="field" for="${id}"><span>${label}</span>${input}</label>`;}
...
function renderInstituteLogin(message="") {
  ...
  document.querySelector("#institute-form").onsubmit=async e=>{
    ...
    try{
      ...
      saveInstituteSession({...state.instituteSession,mustChangePassword:false},remember);state.instituteSession.mustChangePassword=false;state.screen="institute-portal";
      render();
    }catch(err){
...
function renderInstitutePortal(){
  ...
  app.innerHTML=shell(`<section class="card portal-card wide-card gateway-card">
    ...
    <div class="gateway-actions">
      ...
      <button id="gateway-resident" class="gateway-action"><span class="gateway-letter">R</span><div><strong>Resident Home</strong><small>Resident ID and password access</small></div><b>›</b></button>
      <button id="gateway-admin" class="gateway-action"><span class="gateway-letter">D</span><div><strong>Admin Home</strong><small>Institute administration access</small></div><b>›</b></button>
    </div>
  ...
  document.querySelector("#institute-logout").onclick=()=>{clearUiScreen();clearInstituteSession();state.instituteSession=null;state.adminAuthenticated=false;try{sessionStorage.removeItem(ADMIN_SESSION_KEY);sessionStorage.removeItem(SHARE_PASSWORD_KEY);}catch{}state.screen="institute";render();};
...
function renderInstituteAdminLogin(message="") {
 const i=state.instituteSession;if(!i){state.screen="institute";return render();}
 app.innerHTML=shell(`<section class="card login-card"><button id="admin-gate-back" class="back">← Institute Portal</button><div class="card-heading"><span class="step">Institute administration</span><h2>Admin Home</h2><p>Login with the Institute Admin ID and Password.</p></div><form id="institute-admin-login-form">${field("institute-admin-id","Admin ID","text","Enter Admin ID","admin","autocomplete='username'")}${field("institute-admin-password","Admin Password","password","Enter password","","autocomplete='current-password'")}<p id="institute-admin-login-message" class="form-message ${message?"show error":""}">${esc(message)}</p><button id="institute-admin-login-submit" class="primary">Open Admin Home <span>→</span></button></form></section>`,true);
 document.querySelector("#admin-gate-back").onclick=()=>{state.screen="institute-portal";render();};
 document.querySelector("#institute-admin-login-form").onsubmit=async e=>{e.preventDefault();const btn=document.querySelector("#institute-admin-login-submit"),msg=document.querySelector("#institute-admin-login-message");btn.disabled=true;btn.textContent="Checking…";try{await loginInstituteAdmin(i.instituteCode,document.querySelector("#institute-admin-id").value,document.querySelector("#institute-admin-password").value);state.adminAuthenticated=true;try{sessionStorage.setItem(ADMIN_SESSION_KEY,i.instituteCode);}catch{}state.screen="admin-home";render();}catch(err){msg.textContent="Incorrect Admin ID or Password.";msg.className="form-message show error";btn.disabled=false;btn.innerHTML="Open Admin Home <span>→</span>";}};
}
...
function renderAdminLoginSettings(){
 const i=state.instituteSession;if(!i){state.screen="institute";return render();}
 app.innerHTML=shell(`<section class="card dashboard-card wide-card compact-page"><button id="admin-login-settings-back" class="back">← Settings</button><div class="compact-heading"><span class="step">Admin security</span><h2>Change Admin Password</h2><p>Default login is Admin ID <strong>admin</strong> and Password <strong>12345</strong>. Change them here.</p></div><form id="admin-login-settings-form" class="compact-form">${field("new-admin-id","New Admin ID","text","Enter new Admin ID",i.adminId||"admin")}${field("new-admin-password","New Admin Password","password","Minimum 5 characters")}${field("confirm-admin-password","Confirm Password","password","Re-enter password")}<p id="admin-login-settings-message" class="form-message form-wide"></p><button id="admin-login-settings-save" class="primary form-wide">Save Admin Login</button></form></section>`,true);
  ...
  document.querySelector('#branding-form').onsubmit=async e=>{
    ...
    try{
      state.branding=await saveInstituteBranding({
        ...
        contactNumber:document.querySelector('#brand-contact').value,upiId:b.upiId||i.upiId||"",welcomeMessage:document.querySelector('#brand-message').value,
...
function showCredentialsModal(i, password) {
  openModal({ title: "New Login Credentials", eyebrow: "Password reset successful", content: `
    ...
    <p class="modal-note">This password is shown now for secure sharing. The institute may continue with this password or change it later in Settings.</p>
...
function renderStudentLogin(message=""){
  app.innerHTML=shell(`<section class="card login-card"><button id="back-institute-portal" class="back" type="button">← Institute Portal</button><div class="card-heading"><span class="step">Resident access</span><h2>Resident Home</h2><p>Login with the Resident ID and password issued during admission.</p></div><form id="student-login-form">${field("student-login-id","Student ID","text","Enter Student ID","","autocomplete='username'")}${field("student-login-password","Password","password","Enter password","","autocomplete='current-password'")}<p id="student-login-message" class="form-message ${message?"show error":""}">${esc(message)}</p><button id="student-login-submit" class="primary" type="submit">Login <span>→</span></button><p class="student-forgot-note">Forgot password? Contact your Institute Admin.</p></form></section>`,true);
  document.querySelector("#back-institute-portal").onclick=()=>{state.screen="institute-portal";render();};
  document.querySelector("#student-login-form").onsubmit=async e=>{e.preventDefault();const id=document.querySelector("#student-login-id").value,password=document.querySelector("#student-login-password").value,m=document.querySelector("#student-login-message"),b=document.querySelector("#student-login-submit");b.disabled=true;b.textContent="Checking…";try{state.studentSession=await loginStudent(id,password);state.studentCurrentPassword=password;state.studentSession.mustChangePassword=false;saveStudentSession(state.studentSession);state.screen="student-dashboard";render();}catch(err){const messages={"invalid-student-credential":"Incorrect Student ID or password.","student-inactive":"This student account is inactive.","student-profile-missing":"Student profile was not found.","student-login-timeout":"Network is slow. Try again."};m.textContent=messages[err.code]||`Student login failed. ${err.code||""}`;m.className="form-message show error";b.disabled=false;b.innerHTML="Login <span>→</span>";}};
}
...
function isArchivedStudent(s){return Boolean(s?.isArchived)||(s?.accountStatus||s?.status)==="archived";}
function studentAdminRows(){
  const q=state.adminStudentSearch.trim().toLowerCase();
  const items=state.adminStudents
    .filter(s=>state.adminShowArchived?isArchivedStudent(s):!isArchivedStudent(s))
    .filter(s=>!q||`${s.studentName||""} ${s.studentId||""} ${s.parentName||""} ${s.parentPhone||""} ${s.courseOrClass||""}`.toLowerCase().includes(q));
  if(!items.length)return `<div class="empty-state"><strong>${state.adminShowArchived?"No archived students.":"No residents found."}</strong><p>${state.adminShowArchived?"Archived students will appear here and can be restored.":"Create an admission or change the search."}</p></div>`;
...
async function renderInstituteAdmin(){
  const i=state.instituteSession;if(!i){state.screen="institute";return render();}
  app.innerHTML=shell(`<section class="card dashboard-card wide-card"><div class="dashboard-head"><div><span class="step success-step">Resident records</span><h2>Residents</h2><p>${esc(i.instituteName)} · ${esc(i.instituteCode)}</p></div><button id="admin-back-portal" class="secondary">Institute Portal</button></div><div class="metric-grid"><article><span>Total Students</span><strong id="admin-student-total">—</strong><small>Current residents</small></article><article><span>Active</span><strong id="admin-student-active">—</strong><small>Login enabled</small></article><article><span>Capacity</span><strong id="admin-student-capacity">—</strong><small>Residents / Limit</small></article></div><div class="section-title"><div><h3 id="resident-list-title">Residents</h3><p>Search and manage resident details.</p></div><div class="resident-list-actions"><button id="admin-toggle-archived" class="secondary compact-secondary" type="button">Archived</button><button id="admin-new-admission" class="primary compact-primary">New Admission</button></div></div><input id="admin-student-search" class="admin-search" placeholder="Search student, ID, parent, phone or course" value="${esc(state.adminStudentSearch)}"/><p id="admin-student-message" class="form-message"></p><div id="admin-student-results"><div class="loading-card"><div class="loader"></div><p>Loading students…</p></div></div></section>`,true);
  ...
  document.querySelector("#admin-toggle-archived").onclick=()=>{state.adminShowArchived=!state.adminShowArchived;document.querySelector("#resident-list-title").textContent=state.adminShowArchived?"Archived Students":"Residents";const archivedCount=state.adminStudents.filter(isArchivedStudent).length;document.querySelector("#admin-toggle-archived").textContent=state.adminShowArchived?"← Active Residents":`Archived (${archivedCount})`;document.querySelector("#admin-student-results").innerHTML=studentAdminRows();bindStudentAdminRows();};
  ...
  try{state.adminStudents=await listInstituteStudents(i.instituteCode);const current=state.adminStudents.filter(s=>!isArchivedStudent(s)),active=current.filter(s=>(s.accountStatus||"active")==="active"),archived=state.adminStudents.filter(isArchivedStudent);document.querySelector("#admin-student-total").textContent=current.length;document.querySelector("#admin-student-active").textContent=active.length;document.querySelector("#admin-student-capacity").textContent=`${current.length} / ${Number(i.studentLimit||0)||"—"}`;document.querySelector("#admin-toggle-archived").textContent=`Archived (${archived.length})`;document.querySelector("#admin-student-results").innerHTML=studentAdminRows();bindStudentAdminRows();}
...
function renderStudentManage(){
  ...
  document.querySelector("#student-archive").onclick=()=>{if(archived){openModal({title:"Restore Student",eyebrow:"Student lifecycle",content:`<p class="confirm-copy">Restore <strong>${esc(s.studentName)}</strong> to the active Residents list and re-enable student login.</p><p id="modal-message" class="form-message"></p><div class="modal-actions"><button id="cancel-student-restore" class="secondary" type="button">Cancel</button><button id="confirm-student-restore" class="primary" type="button">Restore Student</button></div>`,onReady(){document.querySelector("#cancel-student-restore").onclick=closeModal;document.querySelector("#confirm-student-restore").onclick=async e=>{const b=e.currentTarget;setActionBusy(b,true,"Restoring…");try{await restoreStudentProfile(s.studentId,i.instituteCode);s.accountStatus="active";s.status="active";s.isArchived=false;closeModal();state.adminShowArchived=false;state.screen="institute-admin";render();}catch(err){modalMessage(`Could not restore student. ${err.code||""}`);setActionBusy(b,false);}};}});return;}openModal({title:"Archive Student",eyebrow:"Student lifecycle",tone:"danger",content:`<p class="confirm-copy">This blocks student login and moves the profile out of the active Residents list. Data will not be permanently deleted.</p><label class="field"><span>Type ARCHIVE to confirm</span><input id="student-archive-word" autocomplete="off" placeholder="ARCHIVE"/></label><p id="modal-message" class="form-message"></p><div class="modal-actions"><button id="cancel-student-archive" class="secondary" type="button">Cancel</button><button id="confirm-student-archive" class="primary danger-primary" type="button">Archive Student</button></div>`,onReady(){document.querySelector("#cancel-student-archive").onclick=closeModal;document.querySelector("#confirm-student-archive").onclick=async e=>{if(document.querySelector("#student-archive-word").value.trim().toUpperCase()!=="ARCHIVE")return modalMessage("Type ARCHIVE to continue.");const b=e.currentTarget;setActionBusy(b,true,"Archiving…");try{await archiveStudentProfile(s.studentId,i.instituteCode);s.accountStatus="archived";s.status="archived";s.isArchived=true;closeModal();state.adminShowArchived=false;state.screen="institute-admin";render();}catch(err){modalMessage(`Could not archive student. ${err.code||""}`);setActionBusy(b,false);}};}});};
}




function sharingLabel(room){
  const n=Number(room?.sharingCapacity||room?.capacity||0);
  const type=String(room?.sharingType||"").trim();
  if(type==="single"||n===1)return "Single";
  if(type==="2-sharing"||n===2)return "2 Sharing";
  if(type==="3-sharing"||n===3)return "3 Sharing";
...
function applySelectedRoomFee(room){
  ...
  if(source)source.textContent=fee?`${sharingLabel(room)} fee applied automatically.`:`No fee configured for ${sharingLabel(room)}. Set it in Admission Fees Settings.`;
  if(summary){
  ...
  if(paying){
    ...
    if(!current || current>fee) paying.value=fee||0;
  }
  if(balance && paying)balance.value=Math.max(0,(fee||0)-Number(paying.value||0));
}

function syncAdmissionBalance(prefix="a"){
  const total=document.querySelector(`#${prefix}-total-fees`);
  const paying=document.querySelector(`#${prefix}-paying-now`);
  const balance=document.querySelector(`#${prefix}-balance`);
  if(!total||!paying||!balance)return;
  const fee=Number(total.value||0),paid=Math.max(0,Number(paying.value||0));
  if(paid>fee)paying.value=fee;
  balance.value=Math.max(0,fee-Number(paying.value||0));
}
function ensureAdmissionRoomSelected(){
  if(!state.selectedAdmissionBed){
    throw new Error("Select floor, room and bed before continuing.");
  }
  const fee=sharingFeeForRoom(state.selectedAdmissionBed);
  if(!(fee>0))throw new Error(`No ${sharingLabel(state.selectedAdmissionBed)} fee is configured. Update Admission Fees Settings first.`);
  return fee;
}
...
async function renderStudentNotifications(){
  ...
  try{
    ...
    if(state.screen!=='student-notifications')return;
    list.innerHTML=notes.map(n=>`<button class="notification-row ${n.isRead?'':'unread'}" data-note-id="${esc(n.id)}"><strong>${esc(n.title)}</strong><span>${esc(n.message)}</span><small>${esc(dateTimeText(n.createdAt))} · ${n.isRead?'Read':'New'}</small></button>`).join('')||'<div class="empty-state"><strong>No notifications.</strong></div>';
    document.querySelectorAll('[data-note-id]').forEach(b=>b.onclick=async()=>{
      try{if(!b.dataset.noteId.startsWith('payment-'))await markNotificationRead(b.dataset.noteId);}catch(err){console.warn('Notification read update failed',err);}
      b.classList.remove('unread');
      const note=notes.find(n=>n.id===b.dataset.noteId);
      const small=b.querySelector('small');if(small)small.textContent=`${dateTimeText(note?.createdAt)} · Read`;
    });
  }catch(err){
    console.error('Student notification load failed:',err);
    if(state.screen==='student-notifications')list.innerHTML=`<div class="empty-state"><strong>Notifications could not load.</strong><p>${esc(humanError(err,'Please retry.'))}</p><button id="retry-student-notes" class="secondary compact-button" type="button">Retry</button></div>`;
...
let restoredStudentSession=null;
try{restoredStudentSession=JSON.parse(sessionStorage.getItem(STUDENT_SESSION_KEY)||"null");}catch{}
const savedUiScreen=restoreUiScreen();
if(restoredStudentSession){
  state.studentSession=restoredStudentSession;
  state.screen=STUDENT_SCREENS.has(savedUiScreen)?savedUiScreen:"student-dashboard";
  validateStudentSession(restoredStudentSession.studentId).then(fresh=>{
    state.studentSession={...restoredStudentSession,...fresh};
    saveStudentSession(state.studentSession);
    if(STUDENT_SCREENS.has(state.screen))render();
  }).catch(()=>{
    clearStudentSession();
    clearUiScreen();
...
if(restoredInstitute && !state.studentSession){
  state.instituteSession=restoredInstitute;
  state.screen=(state.adminAuthenticated && ADMIN_SCREENS.has(savedUiScreen))?savedUiScreen:"institute-portal";
  validateInstituteSession(restoredInstitute.instituteCode).then(async fresh=>{state.instituteSession=fresh;state.branding=await getInstituteBranding(fresh.instituteCode).catch(()=>restoredInstitute.branding||null);saveInstituteSession({...fresh,branding:state.branding},true);if(state.screen==="institute-portal")renderInstitutePortal(); else if(ADMIN_SCREENS.has(state.screen))render();}).catch(err=>{clearInstituteSession();state.instituteSession=null;if(state.screen==="institute-portal"){state.screen="institute";renderInstituteLogin(err.code==="subscription-expired"?"Subscription expired. Contact HMOS support.":"Saved institute session expired. Please login again.");}});
}
