import {
  ...
  loginInstituteAdmin, changeInstituteAdminCredentials, checkAdmissionStatus, getSystemHealth, getInstituteLiveMetrics, repairInstituteBedAssignments
} from "./firebase-service.js?v=4.6.0";

const app = document.querySelector("#app");
const HMOS_VERSION = "4.6.0";
...
function printAdmissionStatusDocument(result,type="admission") {
  ...
  const body=type==="receipt"?`<div class="print-grid"><p><b>Receipt No:</b> ${esc(result.receiptNo||"—")}</p><p><b>Student:</b> ${esc(result.studentName||"")}</p><p><b>Student ID:</b> ${esc(result.studentId||"")}</p><p><b>Amount Paid:</b> ₹${Number(result.amountPayingNow||0).toLocaleString("en-IN")}</p><p><b>Balance:</b> ₹${Number(result.balanceAmount||0).toLocaleString("en-IN")}</p><p><b>Transaction ID:</b> ${esc(result.upiTransactionId||"—")}</p></div>`:`<div class="print-grid"><p><b>Student:</b> ${esc(result.studentName||"")}</p><p><b>Student ID:</b> ${esc(result.studentId||"")}</p><p><b>Temporary Password:</b> ${esc(result.temporaryPassword||"Contact Admin")}</p><p><b>Phone:</b> ${esc(result.studentPhone||"")}</p><p><b>Parent:</b> ${esc(result.parentName||"")}</p><p><b>Room / Bed:</b> ${esc(result.floor||"")} · Room ${esc(result.roomNumber||"")} · Bed ${esc(result.bedNumber||"")}</p><p><b>Total Fees:</b> ₹${Number(result.totalFees||0).toLocaleString("en-IN")}</p><p><b>Paid:</b> ₹${Number(result.amountPayingNow||0).toLocaleString("en-IN")}</p><p><b>Balance:</b> ₹${Number(result.balanceAmount||0).toLocaleString("en-IN")}</p></div>`;
  const w=window.open("","_blank");if(!w)return;w.document.write(`<!doctype html><html><head><title>${esc(i.instituteName||"Institute")} - ${title}</title><style>body{font-family:Arial;padding:28px;color:#17324d}.doc{max-width:760px;margin:auto;border:1px solid #d9e2ec;border-radius:18px;padding:28px}h1{margin:0;color:#0b4f8a}.sub{color:#5d6b78;margin:8px 0 24px}.print-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.print-grid p{border:1px solid #e3eaf0;border-radius:10px;padding:12px;margin:0}.foot{margin-top:28px;border-top:1px solid #ddd;padding-top:14px;font-size:12px}@media print{button{display:none}}</style></head><body><section class="doc"><h1>${esc(i.instituteName||result.instituteName||"Institute")}</h1><p class="sub">${esc(i.address||"")} ${i.ownerPhone?" · "+esc(i.ownerPhone):""}</p><h2>${title}</h2>${body}<p class="foot">Powered by HMOS – Hostel Management Operating System</p></section><script>window.onload=()=>window.print()<\/script></body></html>`);w.document.close();
}
...
function renderInstituteAdminHome(){
  ...
  app.innerHTML=shell(`<section class="card portal-card wide-card compact-admin-home">
    ...
    <section class="live-ops-panel" aria-live="polite">
      ...
      <div id="live-metrics-grid" class="live-metrics-grid">
        <button class="live-metric-card" data-metric-screen="institute-admin"><span>Residents</span><strong>—</strong><small>Active · tap to open</small></button>
        <button class="live-metric-card" data-metric-screen="pending-admissions"><span>Pending Admissions</span><strong>—</strong><small>Needs review · tap to open</small></button>
        <button class="live-metric-card" data-metric-screen="entry-exit"><span>Outside</span><strong>—</strong><small>Not returned · tap to open</small></button>
        <button class="live-metric-card" data-metric-screen="room-management"><span>Vacant Beds</span><strong>—</strong><small>Ready to allot · tap to open</small></button>
        <button class="live-metric-card" data-metric-screen="fees-management"><span>Fee Due Today</span><strong>—</strong><small>Accounts · tap to open</small></button>
        <button class="live-metric-card" data-metric-screen="complaints-admin"><span>Open Complaints</span><strong>—</strong><small>Needs action · tap to open</small></button>
      </div>
  ...
  document.querySelectorAll("[data-metric-screen]").forEach(b=>b.onclick=()=>{state.screen=b.dataset.metricScreen;render();});
  ...
  document.querySelector("#approvals-card").onclick=()=>{state.screen="approvals";render();};
  document.querySelector("#notifications-card").onclick=()=>{state.screen="admin-notifications";render();};
  document.querySelector("#audit-card").onclick=()=>{state.screen="audit-logs";render();};
  ...
  const paintLiveMetrics = async () => {
    ...
    try{
      await repairInstituteBedAssignments(i.instituteCode).catch(()=>{});const m=await getInstituteLiveMetrics(i.instituteCode);
      const values=[m.residents,m.pendingAdmissions,m.outsideResidents,m.vacantBeds,m.feeDueToday,m.openComplaints];
      grid.querySelectorAll(".live-metric-card strong").forEach((el,index)=>el.textContent=String(values[index]??0));
...
function renderAdminLoginSettings(){
 const i=state.instituteSession;if(!i){state.screen="institute";return render();}
 app.innerHTML=shell(`<section class="card dashboard-card wide-card compact-page"><button id="admin-login-settings-back" class="back">← Settings</button><div class="compact-heading"><span class="step">Admin security</span><h2>Admin Login Changes</h2><p>Change the Institute Admin ID and password here. Current passwords are never displayed.</p></div><form id="admin-login-settings-form" class="compact-form">${field("new-admin-id","New Admin ID","text","Enter new Admin ID",i.adminId||"admin")}${field("new-admin-password","New Admin Password","password","Minimum 5 characters")}${field("confirm-admin-password","Confirm Password","password","Re-enter password")}<p id="admin-login-settings-message" class="form-message form-wide"></p><button id="admin-login-settings-save" class="primary form-wide">Save Admin Login</button></form></section>`,true);
...
async function renderEntryExit(){
  const i=state.instituteSession;if(!i){state.screen="institute";return render();}
  app.innerHTML=shell(`<section class="card dashboard-card wide-card compact-page"><button id="entry-back" class="back">← Institute Home</button><div class="compact-heading"><span class="step">Movement control</span><h2>Entry / Exit</h2><p>Tap Outside or Returned to view residents. Tap a name for full details.</p></div><div class="mini-stat-grid movement-stat-grid"><button id="outside-stat" class="movement-stat active"><span>Outside Now</span><strong id="outside-count">—</strong><small>Tap to view names</small></button><button id="returned-stat" class="movement-stat"><span>Returned</span><strong id="returned-count">—</strong><small>Tap to view history</small></button></div><div class="compact-section"><h3 id="movement-list-title">Currently Outside</h3><div id="movement-list"><div class="loading-card"><div class="loader"></div></div></div></div></section>`,true);
  ...
  try{
    ...
    state.movements=movements;state.adminStudents=students;
    const outside=movements.filter(x=>x.status==="outside"),returned=movements.filter(x=>x.status==="returned");
    document.querySelector("#outside-count").textContent=outside.length;document.querySelector("#returned-count").textContent=returned.length;
    const list=document.querySelector("#movement-list"),title=document.querySelector("#movement-list-title"),outsideBtn=document.querySelector("#outside-stat"),returnedBtn=document.querySelector("#returned-stat");
    const openDetails=m=>{
      const resident=students.find(s=>normalizeCodeLocal(s.studentId)===normalizeCodeLocal(m.studentId))||{};
      const mapUrl=movementMapsUrl(m);
      openModal({title:m.studentName||resident.studentName||m.studentId,eyebrow:m.status==="outside"?"Currently outside":"Returned resident",wide:true,content:`<div class="movement-detail-grid"><article><span>Reason</span><strong>${esc(m.reason||"—")}</strong></article><article><span>Where</span><strong>${esc(m.location||"—")}</strong></article><article><span>Leaving</span><strong>${esc(`${m.leavingDate||""} ${m.leavingTime||""}`)}</strong></article><article><span>Expected Return</span><strong>${esc(`${m.returnDate||""} ${m.returnTime||""}`)}</strong></article>${m.actualReturnAt?`<article><span>Actual Return</span><strong>${esc(formatDateTime(m.actualReturnAt))}</strong></article>`:""}<article><span>Student ID</span><strong>${esc(m.studentId||"—")}</strong></article></div><div class="movement-actions">${mapUrl?`<button id="view-shared-location" class="primary" type="button">📍 View Location</button>`:"<p class='form-help'>Location coordinates were not shared for this older record.</p>"}${resident.studentPhone?`<a class="secondary button-link" href="tel:${esc(resident.studentPhone)}">Call Resident</a>`:""}${resident.parentPhone?`<a class="secondary button-link" href="tel:${esc(resident.parentPhone)}">Call Parent</a>`:""}${m.status==="outside"?`<button id="detail-mark-entry" class="secondary" type="button">Mark Entry</button>`:""}</div><p id="modal-message" class="form-message"></p>`,onReady(){
        const loc=document.querySelector("#view-shared-location");if(loc)loc.onclick=()=>window.open(mapUrl,"_blank","noopener");
        const mark=document.querySelector("#detail-mark-entry");if(mark)mark.onclick=async()=>{setActionBusy(mark,true,"Marking…");try{await markStudentEntry(m.id);closeModal();renderEntryExit();}catch(err){modalMessage(humanError(err,"Could not mark entry."));setActionBusy(mark,false);}};
      }});
    };
    const paint=(type)=>{
      const data=type==="outside"?outside:returned;
      title.textContent=type==="outside"?"Currently Outside":"Returned History";
      outsideBtn.classList.toggle("active",type==="outside");returnedBtn.classList.toggle("active",type==="returned");
      list.innerHTML=data.map(m=>`<button class="movement-person-row" data-movement="${esc(m.id)}"><div><strong>${esc(m.studentName||m.studentId)}</strong><span>${esc(m.location||"")} · ${esc(m.reason||"")}</span><small>${type==="outside"?`Expected ${esc(`${m.returnDate||""} ${m.returnTime||""}`)}`:`Returned ${esc(formatDateTime(m.actualReturnAt))}`}</small></div><span class="row-chevron">›</span></button>`).join("")||`<div class="empty-state compact-empty"><strong>${type==="outside"?"No one is outside.":"No returned records yet."}</strong></div>`;
      list.querySelectorAll("[data-movement]").forEach(b=>b.onclick=()=>{const m=movements.find(x=>x.id===b.dataset.movement);if(m)openDetails(m);});
    };
    outsideBtn.onclick=()=>paint("outside");returnedBtn.onclick=()=>paint("returned");paint("outside");
  }catch(err){document.querySelector("#movement-list").innerHTML=`<p class="form-message show error">${esc(humanError(err,"Could not load movement records."))}</p>`;}
}
async function renderComplaintsAdmin(){
  const i=state.instituteSession;if(!i){state.screen="institute";return render();}
  app.innerHTML=shell(`<section class="card dashboard-card wide-card compact-page"><button id="complaints-back" class="back">← Institute Home</button><div class="compact-heading"><span class="step">Resident support</span><h2>Complaints</h2><p>Tap a complaint to open full details and update its status.</p></div><div id="complaint-admin-list"><div class="loading-card"><div class="loader"></div></div></div></section>`,true);
  ...
  try{
    ...
    const host=document.querySelector("#complaint-admin-list");
    host.innerHTML=state.complaints.map(c=>`<button class="complaint-summary-row" data-complaint-open="${esc(c.id)}"><div><strong>${esc(c.studentName||c.studentId)}</strong><span>${esc(c.subject||"Complaint")}</span><small>${esc(c.category||"Other")} · ${esc(String(c.status||"submitted").replaceAll("-"," "))}</small></div><span class="row-chevron">›</span></button>`).join("")||'<div class="empty-state compact-empty"><strong>No complaints.</strong></div>';
    host.querySelectorAll("[data-complaint-open]").forEach(b=>b.onclick=()=>{
      const c=state.complaints.find(x=>x.id===b.dataset.complaintOpen);if(!c)return;
      openModal({title:c.subject||"Complaint",eyebrow:"Complaint details",wide:true,content:`<div class="complaint-detail"><article><span>Resident</span><strong>${esc(c.studentName||c.studentId)}</strong><small>${esc(c.studentId||"")}</small></article><article><span>Category</span><strong>${esc(c.category||"Other")}</strong></article><article class="full"><span>Complaint</span><p>${esc(c.details||"")}</p></article><article><span>Submitted</span><strong>${esc(formatDateTime(c.createdAt))}</strong></article></div><label class="field"><span>Status</span><select id="complaint-detail-status"><option value="submitted">Submitted</option><option value="in-review">In Review</option><option value="resolved">Resolved</option><option value="rejected">Rejected</option></select></label><p id="modal-message" class="form-message"></p><button id="save-complaint-status" class="primary" type="button">Save Status</button>`,onReady(){
        const select=document.querySelector("#complaint-detail-status");select.value=c.status||"submitted";
        document.querySelector("#save-complaint-status").onclick=async e=>{const btn=e.currentTarget;setActionBusy(btn,true,"Saving…");try{await updateComplaintStatus(c.id,select.value);closeModal();renderComplaintsAdmin();}catch(err){modalMessage(humanError(err,"Could not update complaint."));setActionBusy(btn,false);}};
      }});
    });
  }catch(err){document.querySelector("#complaint-admin-list").innerHTML=`<p class="form-message show error">${esc(humanError(err,"Could not load complaints."))}</p>`;}
}
function renderPdfReports(){
  const i=state.instituteSession;if(!i){state.screen="institute";return render();}
  const reports=[
    ["all-residents","All Residents PDF"],["pending-fees","Pending Fees PDF"],["fees-next-week","Fees Due Next Week PDF"],
    ["breakfast","Breakfast Attendance PDF"],["lunch","Lunch Attendance PDF"],["dinner","Dinner Attendance PDF"],
    ["outside","Currently Outside PDF"],["complaints","Complaints PDF"]
  ];
  app.innerHTML=shell(`<section class="card dashboard-card wide-card compact-page"><button id="pdf-back" class="back">← Institute Home</button><div class="compact-heading"><span class="step">Reports</span><h2>PDF’s</h2><p>Open a premium report preview, then choose Print / Save as PDF.</p></div><div class="compact-report-grid">${reports.map(([k,r])=>`<button class="secondary compact-report" data-report="${k}">${r}</button>`).join("")}</div><p id="report-message" class="form-message"></p></section>`,true);
  ...
  document.querySelectorAll("[data-report]").forEach(b=>b.onclick=()=>{
    ...
    if(!w){const m=document.querySelector("#report-message");m.textContent="Please allow pop-ups for this site to open report preview.";m.className="form-message show error";return;}
    openInstituteReport(b.dataset.report,w);
  });
}
...
function mealWindow(meal){
  ...
  const windows={
    breakfast:{start:5*60,end:10*60+30,label:"5:00 AM – 10:30 AM"},
    lunch:{start:11*60,end:15*60+30,label:"11:00 AM – 3:30 PM"},
    dinner:{start:18*60,end:22*60+30,label:"6:00 PM – 10:30 PM"},
    night:{start:20*60,end:23*60+59,label:"8:00 PM – 11:59 PM"}
  };
  const w=windows[meal]||{start:0,end:1439,label:"Today"};
  return {...w,open:minutes>=w.start&&minutes<=w.end,date:localDateKey(now)};
}
function formatDateTime(value){
  const d=dateOf(value);
  return d&&!Number.isNaN(d.getTime())?d.toLocaleString("en-IN"):"—";
}
function movementMapsUrl(m){
...
function writeReportDocument(w,{title,subtitle,columns,rows,summary=""}){
  const i=state.instituteSession||{}, b=state.branding||{};
  const instituteName=b.instituteName||i.instituteName||"Institute";
  const logo=(b.shortName||instituteName||"H").trim().charAt(0).toUpperCase();
  const head=columns.map(c=>`<th>${esc(c)}</th>`).join("");
  const body=rows.length?rows.map(r=>`<tr>${r.map(v=>`<td>${esc(v??"—")}</td>`).join("")}</tr>`).join(""):`<tr><td colspan="${columns.length}" class="empty">No records found.</td></tr>`;
  w.document.open();
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${esc(title)} - ${esc(instituteName)}</title><style>
  *{box-sizing:border-box}body{margin:0;background:#f2f6fa;color:#102a43;font-family:Arial,Helvetica,sans-serif}.page{max-width:1050px;margin:22px auto;background:#fff;border:1px solid #d7e1ea;border-radius:22px;overflow:hidden;box-shadow:0 14px 38px rgba(16,42,67,.10)}.hero{padding:26px 30px;background:linear-gradient(135deg,#114b7a,#246b55);color:#fff;display:flex;align-items:center;gap:18px}.logo{width:68px;height:68px;border-radius:18px;background:#fff;color:#246b55;display:grid;place-items:center;font-weight:800;font-size:32px}.hero h1{margin:0 0 4px;font-size:28px}.hero p{margin:0;opacity:.9}.content{padding:28px 30px}.title-row{display:flex;justify-content:space-between;gap:20px;align-items:end;margin-bottom:20px}.title-row h2{margin:0;font-size:26px}.title-row p{margin:6px 0 0;color:#627d98}.stamp{text-align:right;font-size:12px;color:#627d98}.summary{background:#edf6ff;border:1px solid #cbddeb;border-radius:14px;padding:12px 14px;margin-bottom:18px;font-weight:700}table{width:100%;border-collapse:separate;border-spacing:0;border:1px solid #d9e2ec;border-radius:14px;overflow:hidden;font-size:13px}th{background:#eef4f8;text-align:left;padding:11px;border-bottom:1px solid #d9e2ec}td{padding:11px;border-bottom:1px solid #edf2f7;vertical-align:top}tr:last-child td{border-bottom:0}.empty{text-align:center;padding:28px;color:#829ab1}.footer{padding:18px 30px;border-top:1px solid #edf2f7;color:#829ab1;font-size:11px;display:flex;justify-content:space-between}.printbar{padding:14px 30px;background:#fff;display:flex;gap:10px;justify-content:flex-end}.printbar button{border:0;border-radius:12px;padding:12px 18px;font-weight:700;cursor:pointer}.primary{background:#0b5ea8;color:#fff}.secondary{background:#eaf2fb;color:#0b4f8a}@media print{body{background:#fff}.page{box-shadow:none;border:0;margin:0;max-width:none}.printbar{display:none}.content{padding:18px}.hero{print-color-adjust:exact;-webkit-print-color-adjust:exact}table{font-size:11px}th,td{padding:7px}}</style></head><body><section class="page"><div class="hero"><div class="logo">${esc(logo)}</div><div><h1>${esc(instituteName)}</h1><p>${esc(i.instituteCode||"")} · Hostel Management Operating System</p></div></div><div class="content"><div class="title-row"><div><h2>${esc(title)}</h2><p>${esc(subtitle||"")}</p></div><div class="stamp">Generated<br><strong>${esc(new Date().toLocaleString("en-IN"))}</strong></div></div>${summary?`<div class="summary">${esc(summary)}</div>`:""}<table><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div><div class="footer"><span>HMOS Premium Report</span><span>${esc(instituteName)}</span></div><div class="printbar"><button class="secondary" onclick="window.close()">Close</button><button class="primary" onclick="window.print()">Print / Save as PDF</button></div></section></body></html>`);
  w.document.close();
}
async function openInstituteReport(key,w){
  try{
    if(!w)return;
    w.document.write('<p style="font-family:Arial;padding:24px">Preparing report…</p>');
    const i=state.instituteSession;
    const today=localDateKey();
    let columns=[],rows=[],title="",subtitle="",summary="";
    if(key==="all-residents"){
      const students=await listInstituteStudents(i.instituteCode);
      title="All Residents";subtitle="Active resident register";
      columns=["Student","Student ID","Course / Class","Room","Bed","Phone","Parent"];
      rows=students.filter(s=>s.accountStatus!=="archived").map(s=>[s.studentName,s.studentId,s.courseOrClass||"",s.roomNumber?`Room ${s.roomNumber}`:"Not allotted",s.bedNumber?`Bed ${s.bedNumber}`:"—",s.studentPhone||"",s.parentName||""]);
      summary=`${rows.length} resident records`;
    }else if(key==="pending-fees"||key==="fees-next-week"){
      const [students,fees]=await Promise.all([listInstituteStudents(i.instituteCode),listInstituteFees(i.instituteCode)]);
      const sm=new Map(students.map(s=>[normalizeCodeLocal(s.studentId),s]));
      const now=new Date(),week=new Date(now);week.setDate(now.getDate()+7);
      let data=fees.filter(f=>Number(f.balanceAmount||0)>0);
      if(key==="fees-next-week")data=data.filter(f=>{const d=f.dueDate?new Date(`${f.dueDate}T23:59:59`):null;return d&&d>=now&&d<=week;});
      title=key==="pending-fees"?"Pending Fees":"Fees Due Next Week";subtitle="Outstanding resident fee accounts";
      columns=["Student","Student ID","Total","Paid","Balance","Due Date"];
      rows=data.map(f=>{const s=sm.get(normalizeCodeLocal(f.studentId))||{};return [s.studentName||f.studentId,f.studentId,`₹${Number(f.totalFee||0).toLocaleString("en-IN")}`,`₹${Number(f.paidAmount||0).toLocaleString("en-IN")}`,`₹${Number(f.balanceAmount||0).toLocaleString("en-IN")}`,f.dueDate||"—"];});
      summary=`${rows.length} accounts · Outstanding ₹${data.reduce((n,f)=>n+Number(f.balanceAmount||0),0).toLocaleString("en-IN")}`;
    }else if(["breakfast","lunch","dinner"].includes(key)){
      const attendance=await listInstituteMealAttendance(i.instituteCode,today);
      const mealRows=attendance.filter(a=>a.meal===key);
      title=`${key[0].toUpperCase()+key.slice(1)} Attendance`;subtitle=`Meal attendance for ${today}`;
      columns=["Student","Student ID","Status","Date"];
      rows=mealRows.map(a=>[a.studentName||a.studentId,a.studentId,a.status||"present",a.date||today]);
      summary=`${rows.length} marked present`;
    }else if(key==="outside"){
      const movements=await listInstituteMovements(i.instituteCode);
      const data=movements.filter(m=>m.status==="outside");
      title="Currently Outside";subtitle="Residents who have not returned";
      columns=["Student","Reason","Where","Left","Expected Return"];
      rows=data.map(m=>[m.studentName||m.studentId,m.reason||"",m.location||"",`${m.leavingDate||""} ${m.leavingTime||""}`,`${m.returnDate||""} ${m.returnTime||""}`]);
      summary=`${rows.length} residents currently outside`;
    }else if(key==="complaints"){
      const data=await listInstituteComplaints(i.instituteCode);
      title="Complaints";subtitle="Resident complaint register";
      columns=["Student","Category","Subject","Status","Submitted"];
      rows=data.map(c=>[c.studentName||c.studentId,c.category||"Other",c.subject||"Complaint",String(c.status||"submitted").replaceAll("-"," "),formatDateTime(c.createdAt)]);
      summary=`${data.filter(c=>["submitted","in-review"].includes(c.status)).length} open / in review`;
    }
    writeReportDocument(w,{title,subtitle,columns,rows,summary});
  }catch(err){
    w.document.open();w.document.write(`<p style="font-family:Arial;padding:24px;color:#a61b1b">Could not prepare report: ${esc(humanError(err,err?.code||err?.message||"Unknown error"))}</p>`);w.document.close();
  }
}
...
async function renderStudentFees(){
  ...
  const b=await getInstituteBranding(s.instituteCode).catch(()=>null);if(b)state.branding=b;
  const balance=Number(s.feeBalance||0),hasDue=balance>0;
  app.innerHTML=shell(`<section class="card dashboard-card wide-card compact-page">${studentBack()}<div class="compact-heading"><h2>Fees</h2><p>View payment history and pay the outstanding balance.</p></div><div class="mini-stat-grid"><article><span>Total</span><strong>₹${Number(s.feeTotal||0).toLocaleString("en-IN")}</strong></article><article><span>Paid</span><strong>₹${Number(s.feePaid||0).toLocaleString("en-IN")}</strong></article><article><span>Balance</span><strong>₹${balance.toLocaleString("en-IN")}</strong></article></div>${hasDue?paymentBoxHtml(balance,false):'<div class="fully-paid-panel"><strong>✓ Fee Fully Paid</strong><p>No outstanding balance is pending.</p></div>'}<div id="student-payment-history" class="compact-section"></div>${hasDue?`<form id="student-fee-pay-form" class="compact-form">${field("student-pay-amount","Amount to Pay","number","Enter amount",balance,"min='1' max='"+balance+"'")}${field("student-pay-reference","UPI Transaction ID","text","Enter after payment")}<p class="form-message form-wide" id="student-pay-message"></p><button class="primary form-wide">Submit Payment for Approval</button></form>`:""}</section>`,true);
  bindStudentBack();
  try{
    const p=await listStudentPayments(s.studentId);
    document.querySelector("#student-payment-history").innerHTML=`<h3>Payment History</h3>${p.map(x=>`<article class="compact-list-row"><div><strong>₹${Number(x.amount||0).toLocaleString("en-IN")}</strong><span>${esc(x.mode||"UPI")} · ${esc(x.receiptNo||x.reference||"")}</span><small>${esc(x.status||"approved")} · ${esc(formatDateTime(x.paidAt||x.createdAt))}</small></div></article>`).join("")||'<p>No payments yet.</p>'}`;
  }catch{}
  if(!hasDue)return;
  const amountInput=document.querySelector('#student-pay-amount');bindPaymentActions(()=>Number(amountInput.value||0),`${s.studentId} Fee Payment`);
  amountInput.oninput=()=>{const x=document.querySelector('#payment-display-amount');if(x)x.textContent=`₹${Number(amountInput.value||0).toLocaleString('en-IN')}`;};
  document.querySelector("#student-fee-pay-form").onsubmit=async e=>{e.preventDefault();const amount=Number(amountInput.value||0),reference=document.querySelector("#student-pay-reference").value.trim(),m=document.querySelector("#student-pay-message"),btn=e.submitter;if(amount<=0||!reference){m.textContent="Enter payment amount and UPI Transaction ID.";m.className="form-message show error form-wide";return;}if(amount>balance){m.textContent="Amount cannot be greater than the outstanding balance.";m.className="form-message show error form-wide";return;}btn.disabled=true;btn.textContent="Submitting…";try{await submitStudentFeePaymentRequest({studentId:s.studentId,studentName:s.studentName,instituteCode:s.instituteCode,amount,reference});m.textContent="Payment submitted for Admin verification.";m.className="form-message show success-message form-wide";e.target.reset();}catch(err){m.textContent=humanError(err,"Could not submit payment.");m.className="form-message show error form-wide";}finally{btn.disabled=false;btn.textContent="Submit Payment for Approval";}};
}

async function renderStudentAttendance(){
  const s=state.studentSession;if(!s)return renderStudentLogin();
  const meals=["breakfast","lunch","dinner","night"];
  app.innerHTML=shell(`<section class="card dashboard-card wide-card compact-page">${studentBack()}<div class="compact-heading"><span class="step">Daily attendance</span><h2>Attendance</h2><p>Mark meal and night attendance during the available time window.</p></div><div class="meal-card-grid">${meals.map(x=>{const w=mealWindow(x);return `<article class="meal-card"><div><strong>${x==="night"?"Night Present":x[0].toUpperCase()+x.slice(1)}</strong><small>${w.label}</small></div><button class="${w.open?'primary':'secondary'} compact-button" data-meal="${x}" ${w.open?'':'disabled'}>${w.open?'Mark Attendance':'Closed'}</button><p id="meal-status-${x}" class="form-help"></p></article>`}).join("")}</div></section>`,true);
  bindStudentBack();
  for(const meal of meals){
    const w=mealWindow(meal),button=document.querySelector(`[data-meal="${meal}"]`),status=document.querySelector(`#meal-status-${meal}`);
    if(!button)continue;
    try{
      const existing=await getStudentMealAttendance(s.studentId,w.date,meal);
      if(existing){button.disabled=true;button.textContent="Attendance Marked";status.textContent="Marked for today.";}
    }catch(err){status.textContent=humanError(err,"Could not check attendance.");}
    button.onclick=async e=>{
      if(!w.open)return;
      const b=e.currentTarget;b.disabled=true;b.textContent="Saving…";
      try{await submitMealAttendance({studentId:s.studentId,studentName:s.studentName,instituteCode:s.instituteCode,date:w.date,meal});b.textContent="Attendance Marked";status.textContent="Saved successfully.";}
      catch(err){b.disabled=false;b.textContent="Try Again";status.textContent=humanError(err,"Could not save attendance.");}
    };
  }
}
async function renderStudentEntryExit(){
  const s=state.studentSession;if(!s)return renderStudentLogin();
  const now=new Date(),today=localDateKey(now),timeNow=`${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}`;
  let sharedLocation=null;
  app.innerHTML=shell(`<section class="card dashboard-card wide-card compact-page">${studentBack()}<div class="compact-heading"><span class="step">Movement request</span><h2>Entry / Exit</h2><p>Enter your trip details, share your current location and submit.</p></div><form id="student-exit-form" class="compact-form">${field("exit-reason","Reason","text","Reason for going out")}${field("exit-location","Where are you going?","text","Destination / place")}${field("exit-date","Leaving Date","date","",today)}${field("exit-time","Leaving Time","time","",timeNow)}${field("return-date","Expected Return Date","date","",today)}${field("return-time","Expected Return Time","time")}<div class="form-wide location-share-card"><div><strong>Current Location</strong><small id="location-share-status">Location not shared yet.</small></div><button id="share-current-location" type="button" class="secondary compact-button">📍 Share Location</button></div><p id="exit-message" class="form-message form-wide"></p><button id="submit-exit" class="primary form-wide" disabled>Share Location + Submit Exit</button></form><div id="movement-history" class="compact-section"></div></section>`,true);
  bindStudentBack();
  try{
    const rows=await getStudentMovements(s.studentId);
    document.querySelector("#movement-history").innerHTML=`<h3>History</h3>${rows.slice(0,12).map(x=>`<article class="compact-list-row"><div><strong>${esc(String(x.status||"outside").replaceAll("-"," "))}</strong><span>${esc(x.reason)} · ${esc(x.location)}</span><small>${esc(x.leavingDate)} ${esc(x.leavingTime)} → ${esc(x.returnDate)} ${esc(x.returnTime)}${x.actualReturnAt?` · Returned ${esc(formatDateTime(x.actualReturnAt))}`:""}</small></div></article>`).join("")||'<div class="empty-state compact-empty"><strong>No movement history.</strong></div>'}`;
  }catch{}
  const shareBtn=document.querySelector("#share-current-location"),status=document.querySelector("#location-share-status"),submit=document.querySelector("#submit-exit");
  shareBtn.onclick=()=>{
    if(!navigator.geolocation){status.textContent="Location sharing is not supported on this device.";return;}
    shareBtn.disabled=true;shareBtn.textContent="Getting location…";
    navigator.geolocation.getCurrentPosition(pos=>{
      sharedLocation={latitude:pos.coords.latitude,longitude:pos.coords.longitude,locationAccuracy:Math.round(pos.coords.accuracy||0)};
      status.textContent=`Location shared · accuracy about ${sharedLocation.locationAccuracy} m`;
      shareBtn.textContent="✓ Location Shared";submit.disabled=false;submit.textContent="Submit Exit";
    },err=>{
      shareBtn.disabled=false;shareBtn.textContent="📍 Share Location";
      status.textContent=err.code===1?"Location permission denied. Please allow location access and try again.":"Could not get location. Turn on GPS and try again.";
    },{enableHighAccuracy:true,timeout:12000,maximumAge:30000});
  };
  document.querySelector("#student-exit-form").onsubmit=async e=>{
    e.preventDefault();const v=id=>document.querySelector(`#${id}`).value.trim(),msg=document.querySelector("#exit-message");
    if(["exit-reason","exit-location","exit-date","exit-time","return-date","return-time"].some(id=>!v(id))){msg.textContent="Complete all fields.";msg.className="form-message show error form-wide";return;}
    if(!sharedLocation){msg.textContent="Share your current location before submitting.";msg.className="form-message show error form-wide";return;}
    submit.disabled=true;submit.textContent="Submitting…";
    try{
      await submitMovementRequest({studentId:s.studentId,studentName:s.studentName,instituteCode:s.instituteCode,reason:v("exit-reason"),location:v("exit-location"),leavingDate:v("exit-date"),leavingTime:v("exit-time"),returnDate:v("return-date"),returnTime:v("return-time"),...sharedLocation});
      msg.textContent="Exit submitted successfully.";msg.className="form-message show success-message form-wide";submit.textContent="Submitted";
      setTimeout(()=>renderStudentEntryExit(),900);
    }catch(err){msg.textContent=humanError(err,"Could not submit exit.");msg.className="form-message show error form-wide";submit.disabled=false;submit.textContent="Submit Exit";}
  };
}
...
async function renderRoomManagement(){
  ...
  try{
    await repairInstituteBedAssignments(i.instituteCode).catch(err=>console.warn("bed reconciliation",err));[state.rooms,state.adminStudents,state.fees]=await Promise.all([listInstituteRooms(i.instituteCode),listInstituteStudents(i.instituteCode),listInstituteFees(i.instituteCode)]);
...
function openStudentFeeModal(studentId){
  ...
  if(!i||!s)return;
  const total=Number(f.totalFee||0),balance=Number(f.balanceAmount||0),fullyPaid=total>0&&balance<=0;
  const paymentSection=fullyPaid?`<div class="fully-paid-panel"><strong>✓ Fee Fully Paid</strong><p>No outstanding balance. Additional payment entry is disabled to prevent duplicates.</p></div>`:total<=0?`<div class="fully-paid-panel neutral"><strong>No Fee Plan Assigned</strong><p>Set a total fee before recording a payment.</p></div>`:`<hr class="modal-divider"><form id="fee-payment-form" class="form-grid">${field("fee-payment","Payment Amount","number","Enter amount","","min='1' max='"+balance+"'")}<label class="field"><span>Payment Mode</span><select id="fee-mode"><option>Cash</option><option>UPI</option><option>Bank Transfer</option><option>Card</option></select></label>${field("fee-reference","Reference / Note","text","Optional transaction reference")}<p id="modal-message" class="form-message form-wide"></p><button id="fee-payment-save" class="primary form-wide" type="submit">Record Payment & Receipt</button></form>`;
  openModal({title:`Fees · ${s.studentName}`,eyebrow:"Student fee account",wide:true,content:`<div class="fee-summary"><article><span>Total Fee</span><strong>₹${total.toLocaleString("en-IN")}</strong></article><article><span>Paid</span><strong>₹${Number(f.paidAmount||0).toLocaleString("en-IN")}</strong></article><article><span>Balance</span><strong>₹${balance.toLocaleString("en-IN")}</strong></article></div><form id="fee-plan-form" class="form-grid">${field("fee-total","Total Fee","number","Example: 30000",f.totalFee||"","min='0'")}${field("fee-due-date","Due Date","date","",f.dueDate||"")}<button id="fee-plan-save" class="secondary form-wide" type="submit">Save Fee Plan</button></form>${paymentSection}<hr class="modal-divider"><div class="compact-section"><h3>Payment History & Receipts</h3><div id="admin-fee-history"><div class="loading-card"><div class="loader"></div></div></div></div>`,onReady(){
    document.querySelector("#fee-plan-form").onsubmit=async e=>{e.preventDefault();const b=document.querySelector("#fee-plan-save");setActionBusy(b,true,"Saving…");try{await saveStudentFeePlan({studentId,instituteCode:i.instituteCode,totalFee:document.querySelector("#fee-total").value,dueDate:document.querySelector("#fee-due-date").value});state.fees=await listInstituteFees(i.instituteCode);closeModal();renderFeesManagementReady();}catch(err){modalMessage(`Could not save fee plan. ${humanError(err,err.code||"")}`);setActionBusy(b,false);}};
    const paymentForm=document.querySelector("#fee-payment-form");
    if(paymentForm)paymentForm.onsubmit=async e=>{e.preventDefault();const b=document.querySelector("#fee-payment-save");setActionBusy(b,true,"Recording…");try{const receipt=await recordStudentFeePayment({studentId,instituteCode:i.instituteCode,amount:document.querySelector("#fee-payment").value,mode:document.querySelector("#fee-mode").value,reference:document.querySelector("#fee-reference").value});state.fees=await listInstituteFees(i.instituteCode);closeModal();showFeeReceipt(s,receipt);}catch(err){modalMessage(err.code==="payment-exceeds-balance"?"Payment is greater than the outstanding balance.":err.code==="no-outstanding-balance"?"No outstanding balance remains.":humanError(err,"Could not record payment."));setActionBusy(b,false);}};
    listStudentPayments(studentId).then(rows=>{const box=document.querySelector("#admin-fee-history");if(!box)return;box.innerHTML=rows.map(p=>`<article class="compact-list-row"><div><strong>₹${Number(p.amount||0).toLocaleString("en-IN")}</strong><span>${esc(p.mode||"")} · ${esc(p.receiptNo||p.reference||"")}</span><small>${esc(formatDateTime(p.paidAt||p.createdAt))}</small></div></article>`).join("")||'<p>No payments yet.</p>';}).catch(()=>{});
  }});
}
function showFeeReceipt(student,receipt){openModal({title:"Payment Receipt",eyebrow:"Payment recorded successfully",content:`<div class="success-panel"><span>Receipt Number</span><strong>${esc(receipt.receiptNo)}</strong></div><div class="receipt-grid"><article><span>Student</span><strong>${esc(student.studentName)}</strong><small>${esc(student.studentId)}</small></article><article><span>Amount Paid</span><strong>₹${Number(receipt.amount).toLocaleString("en-IN")}</strong><small>${esc(receipt.mode)}</small></article><article><span>Remaining Balance</span><strong>₹${Number(receipt.balanceAmount).toLocaleString("en-IN")}</strong></article></div><div class="modal-actions"><button id="copy-fee-receipt" class="secondary">Copy Receipt</button><button id="print-fee-receipt" class="primary">Print / Save PDF</button></div>`,onReady(){const text=`HMOS Fee Receipt\nReceipt: ${receipt.receiptNo}\nStudent: ${student.studentName} (${student.studentId})\nPaid: ₹${receipt.amount}\nBalance: ₹${receipt.balanceAmount}\nMode: ${receipt.mode}`;document.querySelector("#copy-fee-receipt").onclick=()=>copyText(text);document.querySelector("#print-fee-receipt").onclick=()=>{const w=window.open("","_blank");if(!w)return;writeReportDocument(w,{title:"Payment Receipt",subtitle:`Receipt ${receipt.receiptNo}`,columns:["Student","Student ID","Amount","Mode","Balance"],rows:[[student.studentName,student.studentId,`₹${Number(receipt.amount).toLocaleString("en-IN")}`,receipt.mode||"",`₹${Number(receipt.balanceAmount).toLocaleString("en-IN")}`]],summary:`Receipt No: ${receipt.receiptNo}`});};}});}
...
async function renderBackupRestore(){
  ...
  restoreButton.onclick=async()=>{if(!verifiedBackup||confirmWord.value.trim().toUpperCase()!=='RESTORE')return;restoreButton.disabled=true;restoreButton.textContent='Creating safety backup…';restoreMsg.textContent='Do not close this page. Preparing a pre-restore safety copy.';restoreMsg.className='form-message show info';try{const safety=await exportInstituteBackup(i.instituteCode);downloadPayload(safety,'HMOS-PRE-RESTORE');await createBackupSnapshot(i.instituteCode);restoreButton.textContent='Restoring records…';const result=await restoreInstituteBackup(verifiedBackup,i.instituteCode);restoreMsg.textContent=`Recovery completed safely. ${result.restoredRecords} records restored across ${result.restoredCollections} collections. ${result.skippedNewerRecords||0} newer live records were preserved.`;restoreMsg.className='form-message show success-message';confirmWord.value='';verifiedBackup=null;controls.hidden=true;fileInput.value='';out.innerHTML='';await paintHistory();}catch(err){restoreMsg.textContent=`Recovery stopped: ${humanError(err,err.code||err.message||'Unknown error')}`;restoreMsg.className='form-message show error';restoreButton.disabled=false;restoreButton.textContent='Restore Verified Backup';}};
