...
export async function recordStudentFeePayment({studentId,instituteCode,amount,mode,reference}){
  ...
  try {
    await createNotification({instituteCode,recipientType:"resident",recipientId:studentId,title:"Fee payment received",message:`₹${amount.toLocaleString("en-IN")} payment recorded. Receipt ${receiptNo}. Remaining balance ₹${Number(result.balanceAmount||0).toLocaleString("en-IN")}.`,type:"payment",relatedType:"payments",relatedId:result.paymentId});
  } catch (err) { console.warn("payment notification", err); }
  return result;
}
...
export async function submitPendingAdmission(input, instituteSession) {
  ...
  const upiTransactionId=cleanText(input.upiTransactionId).replace(/\s+/g,"").toUpperCase();
  ...
  const data={...input,upiTransactionId,applicationId,instituteId,instituteCode,instituteName:cleanText(instituteSession.instituteName),roomNumber:room.roomNumber,floor:room.floor,building:room.building,totalFees,amountPayingNow,balanceAmount:Math.max(0,totalFees-amountPayingNow),status:"pending_payment_verification",createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
  const batch=writeBatch(db);
...
export async function submitMovementRequest(input){
  ...
  try{await createNotification({instituteCode,recipientType:"admin",title:"Resident outside",message:`${data.studentName||studentId} submitted an exit to ${data.location}. Expected return ${data.returnDate} ${data.returnTime}.`,type:"movement",relatedType:"movements",relatedId:ref.id});}catch(err){console.warn("exit notification",err);}
  return data;
}
export async function markStudentEntry(movementId){const id=cleanText(movementId),ref=doc(db,"movements",id),snap=await getDoc(ref);if(!snap.exists())throw Object.assign(new Error("Movement not found"),{code:"movement-not-found"});const m=snap.data();await updateDoc(ref,{status:"returned",actualReturnAt:serverTimestamp(),updatedAt:serverTimestamp()});try{await createNotification({instituteCode:m.instituteCode,recipientType:"resident",recipientId:m.studentId,title:"Entry recorded",message:"Your return to the institute has been recorded.",type:"movement",relatedType:"movements",relatedId:id});}catch(err){console.warn("entry notification",err);}return true;}
...
export async function updateComplaintStatus(complaintId,status){const id=cleanText(complaintId),ref=doc(db,"complaints",id),snap=await getDoc(ref);if(!snap.exists())throw Object.assign(new Error("Complaint not found"),{code:"complaint-not-found"});const complaint=snap.data(),nextStatus=cleanText(status);await updateDoc(ref,{status:nextStatus,updatedAt:serverTimestamp()});await createAuditLog({instituteCode:complaint.instituteCode,actorType:"admin",action:"complaint_status_changed",entityType:"complaints",entityId:id,summary:`${complaint.subject||"Complaint"}: ${complaint.status||"submitted"} → ${nextStatus}`,oldValue:{status:complaint.status||"submitted"},newValue:{status:nextStatus}});try{await createNotification({instituteCode:complaint.instituteCode,recipientType:"resident",recipientId:complaint.studentId,title:"Complaint status updated",message:`${complaint.subject||"Complaint"} is now ${nextStatus.replaceAll("-"," ")}.`,type:"complaint",relatedType:"complaints",relatedId:id});}catch(err){console.warn("complaint notification",err);}return true;}
...
export async function saveInstituteBranding(input){
  ...
  const data={instituteCode,instituteId:cleanText(input.instituteId),instituteName:cleanText(input.instituteName),shortName:cleanText(input.shortName),logoUrl:cleanText(input.logoUrl),primaryColor:cleanText(input.primaryColor)||"#0b4f8a",secondaryColor:cleanText(input.secondaryColor)||"#16866b",contactNumber:cleanText(input.contactNumber).replace(/\D/g,""),upiId:cleanText(input.upiId),defaultTotalFees:Number(input.defaultTotalFees||0),welcomeMessage:cleanText(input.welcomeMessage),dashboardMode:["hmos","institute","custom"].includes(cleanText(input.dashboardMode))?cleanText(input.dashboardMode):"custom",fontScale:Math.min(1.3,Math.max(.85,Number(input.fontScale||1))),cardRadius:Math.min(40,Math.max(12,Number(input.cardRadius||24))),updatedAt:serverTimestamp()};
  await setDoc(doc(db,"instituteBranding",instituteCode),data,{merge:true});
...
export async function listNotifications({instituteCode,recipientType,recipientId=""}){
  const code=normalizeCode(instituteCode),rid=normalizeCode(recipientId),q=query(collection(db,"notifications"),where("instituteCode","==",code),limit(500)),snap=await getDocs(q);
  const accepted=recipientType==="resident"?["resident","student"]:[recipientType];
  const notes=snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>accepted.includes(x.recipientType)&&(!rid||normalizeCode(x.recipientId)===rid));
  // Backfill visibility for payments created by older builds that did not create resident notifications.
  if(recipientType==="resident"&&rid){
    try{
      const ps=await getDocs(query(collection(db,"payments"),where("studentId","==",rid),limit(250)));
      const related=new Set(notes.map(n=>cleanText(n.relatedId)));
      for(const d of ps.docs){const x=d.data();if(normalizeCode(x.instituteCode)!==code||x.status==="pending_verification"||related.has(d.id))continue;const receipt=cleanText(x.receiptNo);if(receipt&&notes.some(n=>cleanText(n.message).includes(receipt)))continue;notes.push({id:`payment-${d.id}`,instituteCode:code,recipientType:"resident",recipientId:rid,title:"Fee payment received",message:`₹${Number(x.amount||0).toLocaleString("en-IN")} payment recorded${receipt?`. Receipt ${receipt}`:""}.`,type:"payment",relatedType:"payments",relatedId:d.id,isRead:true,createdAt:x.createdAt||x.paidAt});}
    }catch(err){console.warn("payment notification backfill",err);}
  }
  return notes.sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));
}
